...
uvx showboat note "$DEMO_FILE" "The home page with navigation cards. Dark theme, retro sports broadcast aesthetic."
...
uvx showboat note "$DEMO_FILE" "Click into a game for box scores and rich narrated play-by-play with player names and defenders. Every possession is recorded."
...
uvx showboat exec "$DEMO_FILE" bash "uv run python scripts/demo_seed.py status"
...
uvx showboat note "$DEMO_FILE" "All 408 tests pass. Zero lint errors. The demo above was captured live from a running instance."

uvx showboat exec "$DEMO_FILE" bash "uv run pytest --tb=short -q 2>&1 | tail -3"
