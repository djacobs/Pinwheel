...
| **Report** | An AI-generated reflection on gameplay or Floor patterns. Never prescriptive. Types: simulation, Floor, private, tiebreaker, series, season, offseason, State of the League. The AI agent that generates them is the **Reporter**. | "Reflection" | "report", "analysis", "feedback" |
...
| **Tally Round** | A Round where governance tallying occurs. Unresolved Proposals are voted on, passed rules are enacted, and tokens regenerate. Frequency controlled by `PINWHEEL_GOVERNANCE_INTERVAL` (default every 3 Rounds, governable). Proposals, amendments, votes, and trades happen asynchronously between any rounds — the tally round is when they resolve. | "The Floor", "governance round" | "window" (there is no time-bounded window), "session", "voting period" |
