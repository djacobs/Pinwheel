...
## Completed — Session 138 (Granular Sim Engine Phases 1+2)

### 149. [DONE] Subtype-aware play-by-play narration
**Problem:** Every rim shot narrated from the same 3-4 generic templates regardless of what actually happened — a dunk, a floater, and a putback all read as "takes it strong to the hole." Blocks never appeared in narration (the stat was never even recorded), travels didn't exist, and a foul on a made shot awarded nothing and read like a plain make.
**Fix:** The possession engine now emits a granular event chain (`PossessionLog.events`) and narration consumes it: `narrate_play` gains `subtype`/`and_one`/`blocked` params with dedicated template pools for dunks ("rises and THROWS IT DOWN"), floaters, putbacks, tips, stepbacks, fadeaways, catch-and-shoot, corner threes, heaves, blocked shots ("REJECTED by {defender}!"), travels, double dribbles, and bad-pass vs lost-ball turnovers; made-shot fouls append "— and one!". `extract_event_context()` pulls the context from either GameEvent models (presenter/SSE) or DB JSON dicts (game detail page). Unknown subtypes and pre-enrichment possessions fall back to the exact templates used before, and play-by-play stays one line per possession — richer, not noisier.

## Completed — Session 141 (Granular Sim Engine Phase 5)

### 151. [DONE] Transition possessions narrate "On the break" + Tier-3 event narration
**Problem:** Transition possessions (opened by a live-ball turnover or defensive rebound) simulated differently — faster initiation, at-rim bias, transition shot tags — but the play-by-play summary line read identically to a half-court set. And the new Tier-3 governance surface (backcourt/three-second/lane/kicked-ball/goaltending violations, take and clear-path fouls, check-ball and clear-arc restarts, injuries) had no narration, so activating those rules would have produced generic fallback lines.
**Fix:** Summary `PossessionLog` rows now carry a possession-level `tags` field; the presenter and game-detail page pass `transition="transition" in tags` into `narrate_play`, which prefixes shot narration with "On the break — ". Every Tier-3 event type gets dedicated templates in `narrate_event` and the violation pools ("Whistle — {player} brings it back over half court, backcourt violation", "GOALTEND — {actor} swats it on the way down, basket counts", "{actor} takes the foul — the break dies right there", "{actor} comes up hobbling — gutting it out on one good leg"); `foul.*` events with templates no longer collapse to the generic whistle line. Default games are visually unchanged — the tag only decorates possessions that actually opened in transition, and Tier-3 lines only appear once governance activates those rules.

## Completed — Session 140 (Granular Sim Engine Phase 4)

### 150. [DONE] Expandable per-possession event chain on the game page
**Problem:** With the micro engine live (Phase 4), every possession is now a chain of granular events — passes, screens, cuts, drives, closeouts, steal gambles — but the game page's play-by-play showed only the one-line summary. The richer story existed in the data and had no surface; dumping it inline would have buried the summary in noise.
**Fix:** The default rendering stays ONE line per possession. Possessions whose chain has more than one event wrap the summary line in a plain HTML `<details>`/`<summary>` expander (no JS build step) with an "N events" hint; expanding reveals the chain as an indented, left-ruled list of short narrated lines ("Ayo swings it around the perimeter", "Bex closes out hard — contested", "Loose ball! Kai comes up with it") via the new `narrate_event()` in `core/narrate.py`, which covers every Tier-2 event family and falls back gracefully on unknown/governance-added event types. The presenter includes the same narrated chain in each `presentation.possession` SSE payload (`events: [{type, outcome, text}]`, capped at 32) so live surfaces can adopt the expander without re-deriving narration. Chain-less legacy possessions render exactly as before, and the click-to-hooper affordance moved to the narration span.
