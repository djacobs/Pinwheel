...
This loop runs continuously. Games simulate on a cron schedule. Governance actions (proposals, votes, trades) happen asynchronously via Discord at any time. Every Nth round, unresolved proposals are tallied and enacted. The AI reflects after each round and after each tally. The tight loop means you never have to wait long to see what your decisions did.

## Round Cadence

The system advances one round at a time, paced by `PINWHEEL_PRESENTATION_PACE`:

| Pace | Round interval | Use case |
|------|---------------|----------|
| `fast` | 1 min | Local dev, rapid testing |
| `normal` | 5 min | Demo, staging |
| `slow` | 15 min | Production play |
| `manual` | Admin-triggered | Controlled demos |

Each round:

1. **Simulate** — 2-4 games run instantly (one per matchup). Results stored.
2. **Tally** (every Nth round) — Unresolved proposals are voted on. Passed rules enacted. Tokens regenerate.
3. **Report** — Simulation report generated. On tally rounds, governance and private reports also generate.
4. **Present** — In replay mode, play-by-play streams to fans via SSE over real time. In instant mode, results appear immediately.
5. **Notify** — After presentation finishes, Discord gets game results, commentary, and (on tally rounds) governance outcomes.

Governors can propose, vote, amend, and trade at any time between rounds via Discord slash commands. The tally round is when those actions resolve. The default governance interval is every 3 rounds (`PINWHEEL_GOVERNANCE_INTERVAL=3`), but this is itself a governable parameter — players can vote to make it more or less frequent.

*For hackathon demo: fast pace (1 min rounds), governance interval 1 (tally every round). A judge sees a full govern→simulate→observe→reflect cycle in under 5 minutes.*
...
Tokens regenerate on each governance tally round: 2 of each type per tally cycle. They're tradeable between any players — teammates or opponents. This creates a political economy around governance itself.

Token trades are public (the governance report will note them) but the terms are private. You can see that Player A traded with Player B, but not what was exchanged. Unless someone tells the group — or the AI notices a pattern.
...
6. **Vote:** All players vote via `/vote` in Discord. BOOST tokens can be spent to double vote weight. Simple majority passes (unless the players have changed this meta-rule). Votes resolve on the next tally round.
...
### Layer 1: Simulation Report (Shared)
...
### Layer 2: Governance Report (Shared)
...
### Layer 3: Private Report (Per-Player)
...
The private report never prescribes. It reflects. The player decides what to do with the information.
...
The social game of governance depends on where conversations happen and who can see them. Pinwheel's feed architecture has three layers, each serving a different function. The AI observes all three layers (to different degrees) so that the report system can surface dynamics that span them.

**Layer 1: The Public Square (League-Wide Feed)**
AI simulation and governance report observations, game highlights, proposal announcements, vote results, and open debate. Visible to all players and spectators. This is where coalitions are forged in public, where persuasion happens, and where the governance report posts its shared analysis. On Discord, this maps to #announcements, #game-day, #governance-floor, and #reports.
...
If a governor doesn't vote before the tally round, their vote is not cast. Their team's vote weight redistributes among governors who did vote. If no governors on a team vote, that team's weight drops to zero for that tally and total possible vote weight decreases accordingly. The report tracks inactivity patterns.
...
2. **The satisfaction of insight.** The private report tells you something about yourself you didn't see. You change your approach. It works.
