...
- Each round has its own section header ("Round 3") and associated mirror card
- Users can catch up on a day's games at a glance
- Updated `arena_page()` in `pages.py` — data structure changed from flat `games` list to `rounds` list of `{round_number, games, mirror}` dicts

### Narrative Mock Mirrors
- Rewrote `generate_simulation_mirror_mock()` — now uses team names, scores, margins, and game-specific details instead of generic "Round N delivered M games with X points"
...
**Files modified (7):** `config.py`, `api/pages.py`, `ai/mirror.py`, `templates/base.html`, `templates/pages/arena.html`, `templates/pages/standings.html`, `static/css/pinwheel.css`
...
- **Mirror content** changed from `white-space: pre-wrap` to `white-space: normal` + `max-width: 720px`

### Demo Script Update
- All `.venv/bin/python` → `uv run python` (9 occurrences)
- Test count 327 → 408
- Arena screenshot height 900 → 1400
- Updated descriptions for arena (multi-round + narration), game detail (narrated play-by-play), mirrors (narrative)
