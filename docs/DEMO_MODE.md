...
|-----------|-------------|---------|------------|
| `PINWHEEL_GAME_CRON` | `*/2 * * * *` (every 2 min) | `*/2 * * * *` (every 2 min) | `0 * * * *` (hourly) |
| `PINWHEEL_GOV_WINDOW` | `120` (2 min) | `120` (2 min) | `1800` (30 min) |
| `PINWHEEL_PRESENTATION_PACE` | `fast` | `demo` or `fast` | `production` |

---

## 3. Pace Modes

The Presenter paces pre-computed game results through SSE events. Four modes control the base interval between possessions:

| Mode | Base Interval | Game Duration | Use Case |
|------|--------------|---------------|----------|
| `instant` | 0s | ~0s | Tests, batch simulation, 1000-game runs |
| `demo` | 5s | ~2-3 min | Live demos, hackathon judging |
| `fast` | 15s | ~5-8 min | Local development, staging |
| `production` | 60s | ~20-30 min | Real league play |

**Dramatic pacing modifiers** adjust the base interval dynamically:

| Situation | Multiplier | Effect |
|-----------|-----------|--------|
| Blowout (diff >= 15) | x 0.6 | Speed up — less interesting |
| Run (5-0+ scoring run) | x 0.8 | Slightly faster — momentum |
| Lead change | x 1.3 | Slow down — dramatic |
| Elam activated | x 2.0 | Much slower — tension building |
| Final 5 possessions (close game) | x 1.5 | Slower — endgame |
| Final possession | x 2.0 | Slowest — climax |

```python
# Pace mode definitions
PACE_MODES = {
    "instant":    PaceMode(name="instant",    base_interval=0.0),
    "demo":       PaceMode(name="demo",       base_interval=5.0),
    "fast":       PaceMode(name="fast",       base_interval=15.0),
    "production": PaceMode(name="production", base_interval=60.0),
}
```
...
| Governance interval | Every round | Every round (governable) |
...
| Mirror staleness tolerance | Infinite (never stale) | 1 round | 1 round |
...
| 4:00 | Show Mirror delivery | Simulation mirror posts. Show governance mirror connecting rules to outcomes. |
| 4:30 | Show private mirror (DM) | The thesis moment: "Here's what the AI sees about YOUR governance pattern." |
...
- **Mirror:** Governance mirror references the sharpshooter-favoring rule changes. Private mirror to the proposing governor notes their pattern.
...
| `PINWHEEL_GAME_CRON` | No | `*/2 * * * *` (dev) / `0 * * * *` (prod) | Cron schedule for simulation blocks |
| `PINWHEEL_GOV_WINDOW` | No | `120` (dev) / `1800` (prod) | Seconds per governance window |
| `PINWHEEL_PRESENTATION_PACE` | No | `fast` (dev) / `production` (prod) | Pace mode: `instant`, `demo`, `fast`, `production` |
